...
class MuTriModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            if opt.criterionHCG == 'L1':
                self.criterionHCG = torch.nn.L1Loss()
            elif opt.criterionHCG == 'L2':
                self.criterionHCG = torch.nn.MSELoss()
            elif opt.criterionHCG == 'SSIM':
                class SSIMLoss(SSIM):
                    def forward(self, x, y):
                        x3 = ((x+1.)/2.).repeat(1,3,1,1)
                        y3 = ((y+1.)/2.).repeat(1,3,1,1)
                        return 1. - super().forward(x3, y3)
                self.criterionHCG = SSIMLoss().cuda()
            if opt.criterionVPG == 'L1':
                self.criterionVPG = torch.nn.L1Loss()
            elif opt.criterionVPG == 'L2':
                self.criterionVPG = torch.nn.MSELoss()
            elif opt.criterionVPG == 'BCE':
                self.criterionVPG = torch.nn.CrossEntropyLoss()
    ...
    def backward_G(self):
        ...
        #print("HCG output",self.fake_B_proj_s.size(),self.fake_B_proj_t.size()) # HCG output: [1,1,256,256]
        print("VPG output",self.fake_B_seg.unsqueeze(0).size(),self.real_B_seg[1].unsqueeze(0).size()) # VPG output: [2,256,256]

        # Third, proj(G(A)) = proj(B)
        self.loss_G_L1_pm = self.criterionL1(self.fake_B_proj_s, self.real_B_proj) * self.opt.lambda_C
        self.loss_G_L1_pm_st = self.criterionHCG(self.fake_B_proj_s, self.fake_B_proj_t) * self.opt.lambda_C
        if self.opt.criterionVPG == 'BCE':
            self.loss_G_L1_seg = self.criterionVPG(self.fake_B_seg.unsqueeze(0), self.real_B_seg[1].unsqueeze(0).type(torch.LongTensor).cuda()) * self.opt.lambda_C
        else:
            self.loss_G_L1_seg = self.criterionVPG(self.fake_B_seg, self.real_B_seg) * self.opt.lambda_C
