...
class Codebook(nn.Module):
    def __init__(self,  latent_dim, num_codebook_vectors, resolution=448):
        ...
        self.hasposition = False  # False before args.hasposition 
        self.lora_flag = False #True 
        ...
        if self.lora_flag == False:
            self.embedding = nn.Embedding(self.num_codebook_vectors, latent_dim)
            self.embedding.weight.data.uniform_(-1.0 / self.num_codebook_vectors, 1.0 / self.num_codebook_vectors)
        else:
            self.embedding = lora.Embedding(self.num_codebook_vectors, latent_dim, r=16)
